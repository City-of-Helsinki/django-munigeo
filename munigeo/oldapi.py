...
def srid_to_srs(srid):
    ...
    try:
        ...
        raise InvalidFilterError(
            "SRID %d not found (try 4326 for GPS coordinate system)" % srid
        )
    return srs
...
class AdministrativeDivisionResource(TranslatableCachedResource):
    ...
    def alter_list_data_to_serialize(self, request, bundles):
        ...
        data["features"] = [
            self._convert_to_geojson(bundle) for bundle in bundles["objects"]
        ]
        return data
    ...
    class Meta:
        queryset = (
            AdministrativeDivision.objects.order_by("ocd_id")
...
class AddressResource(ModelResource):
    municipality = fields.ToOneField(
        "munigeo.api.MunicipalityResource",
        "municipality",
    ...
    def build_filters(self, filters=None):
        ...
        if filters:
            ...
            if "distinct_streets" in filters:
                orm_filters.update(self.distinct_streets(filters["distinct_streets"]))
        return orm_filters
...
all_resources = [
    ...
    AdministrativeDivisionTypeResource,
    AddressResource,
    POICategoryResource,
    POIResource,
